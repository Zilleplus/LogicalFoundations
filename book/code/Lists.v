...
Fixpoint count (v : nat) (s : bag) : nat := 
  match s with
  | h::t => (count v t) + (if (h =? v) then 1 else 0)
  | nil => 0
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  | nil => nil
  | h::t => (if h=?v then [] else [h] ) ++ (remove_all v t)
  end.
...
Fixpoint subset (s1 : bag) (s2 : bag) : bool :=
  match s1 with
  | nil => true
  | h :: t => ((count h t) + 1 <=? (count h s2)) && (subset t s2)
  end.
...
Theorem bag_theorem : forall (a : nat) (b : bag),
  (count a b)+ 1 = count a ( add a b ).
Proof.
  intros a b.
  assert(G: a=?a = true). 
  { induction a. 
...
(** There is a short solution to the next one.  If you find yourself
    ...
    way. *)
(* assert(G : (n::l1) ++ l2 = [n] ++ l1 ++ l2). *)

Theorem app_assoc4 : forall l1 l2 l3 l4 : natlist,
  l1 ++ (l2 ++ (l3 ++ l4)) = ((l1 ++ l2) ++ l3) ++ l4.
Proof.
  intros l1 l2 l3 l4.
  assert (G: (l1 ++ l2)++l3 = l1 ++ l2 ++ l3). { rewrite -> app_assoc. reflexivity. }
  rewrite -> G.
...
Theorem remove_head_from_natlist : forall (n : nat) (l1 l2 : natlist), 
  (n::l1) ++ l2 = [n] ++ l1 ++ l2.
Proof.
  simpl. reflexivity.
Qed.
...
Proof.
  ...
  induction l1. 
  - simpl. reflexivity.
  - rewrite -> remove_head_from_natlist. 
    induction n.
...
Theorem bag_theorem_head_tail_match : forall (a : nat) (b :bag),
  count a b + 1 = count a (a::b).
Proof.
  simpl.
  intros a.
  assert(G: (a=?a) = true). { induction a. -reflexivity. -simpl. rewrite -> IHa. reflexivity. }
  rewrite -> G.
  reflexivity.
Qed.
...
Proof.
  intros s.
  rewrite <- bag_theorem_head_tail_match.
  rewrite <- plus_comm.
  simpl.
  reflexivity.
Qed.
...
Proof.
  ...
  - reflexivity.
  - induction n.
    -- simpl. rewrite -> plus_comm. simpl. rewrite -> leb_n_Sn. reflexivity.
    -- simpl. rewrite <- plus_comm. simpl. rewrite <- plus_comm. simpl. rewrite -> IHs. reflexivity.
Qed.
(** [] *)
...
(* every baz contains an other bazz, these are infinite elements in bazz *)
