...
(** **** Exercise: 1 star, advanced (fold_types_different) 
    ...
    (* When you want to execute a lambda and fold the result *)
...
(** Write down a theorem [fold_map_correct] in Coq stating that
   ...
   [simpl].) *)
Theorem fold_map_dist  : forall  X Y (f: X -> Y) (l: list X ) (x: X),
...
*)
Theorem minus_zero_is_the_same : forall n : nat,
  n - 0  = n.
Proof.
  intros n.
  induction n.
...
Theorem empty_has_no_elements : forall (X: Type) (l : list X),
  @nth_error X l (length l) = None.
Proof.
  intros X l.
  induction l.
  - reflexivity.
  - simpl. rewrite -> minus_zero_is_the_same.
    rewrite -> IHl.
    reflexivity.
Qed.


Theorem nth_error_informal : forall (X : Type) (l: list X) (n: nat),
  length l = n -> @nth_error X l n = None.
Proof.
  intros X l n H.
  rewrite <- H.
  rewrite -> empty_has_no_elements.
  reflexivity.
Qed.
