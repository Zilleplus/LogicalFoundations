...
(* FILL IN HERE
rewrite only uses stuff from one side, while an apply uses an whole equality.
    [] *)
...
(* 
    We want to proof that if the equality n=?m is true, n and m must be equal.(makes sence)

    so doing induction on n

    if n=0
  -  0 =? m  -> destruct m
     so if m = 0 , m and n are 0, so m=n is true
     if m = S m' then we get 0 ?= S m' which by defintion can't happen so whatever follows is alway's we can assume to be alway's true...
 - if n = S n' 
   destruct m so 
    if m=0 -> we get the same S n'=0 nonssence...
    if m=S m' we get S n' =? S m' which we can simplify to n' = m'
       -> the remaining S n' = S m' can be transformed to n' = m' using the injection property.
...
intros X Y l.
  ...
  - (* l = nil *) intros l1 l2 H. injection H. intros eq1 eq2. rewrite <- eq1. rewrite <- eq2. reflexivity.
  - (* l = n :: l' *)
    destruct n as (x, y).
    simpl.
    destruct (split l').
    ...
    + (* l1 = nil *) discriminate H2.
    + (* l1 = m :: l1' *)
      inversion H1.   (* got this from some random github... not sure what this does... *)
      inversion H2. simpl.
...
Proof.
  ...
  destruct (f true) eqn:H1.
      - destruct (f false) eqn:H2.
        * destruct b.
          + rewrite -> H1. rewrite -> H1. rewrite -> H1. reflexivity.
          + rewrite -> H2. rewrite -> H1. rewrite -> H1. reflexivity.
        * destruct b.
          + rewrite -> H1. rewrite -> H1. rewrite -> H1. reflexivity.
          + rewrite -> H2. rewrite -> H2. rewrite -> H2. reflexivity.
      - destruct (f false) eqn:H2.
        * destruct b.
...
Proof.
  ...
  - induction m.
    * reflexivity.
    * apply zero_nbeq_S.
  - simpl. destruct m eqn:IHm.
    * rewrite -> zero_nbeq_S. reflexivity.
    * simpl. apply IHn.
Qed.
...
Theorem zero_len_list : forall (X:Type) (l : list X),
  0=length l -> l = [].
Proof.
  intros X.
  induction l.
...
Proof.
  intros X.
  induction l1.
  - simpl.  intros l2 H. f_equal. apply zero_len_list in H. symmetry. apply H.
  - induction l2.
    * simpl. discriminate.
    * simpl. intros eq. rewrite -> IHl1. reflexivity.  injection eq as eq2.  apply eq2.
Qed.
...
Proof.
   induction l.
   - simpl. discriminate.
   - simpl. intros lf. (* rewrite -> IHl.  doesn't seem to work this is super weird*)
Admitted.
(** [] *)
